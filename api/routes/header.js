...
var upload = multer({
    storage: multerS3({
        ...
        key: function (req, file, cb) {
            console.log(file);
            const fileName = 'header/'+Date.now()+"-"+file.originalname.toLowerCase().split(' ').join('-');
...
router.get("/category", async function(req, res, next) {
  try {
    const results = await db.query("SELECT id, title FROM categories where status=1 order by title asc");
    return res.json(results.rows);
  } catch (err) {
    return next(err);
  }
...
router.get("/category-relation", async function(req, res, next) {
  try {
    const results = await db.query("SELECT id, category_group_id, category_ids FROM category_group_relation order by id asc");
    return res.json(results.rows);
  } catch (err) {
    return next(err);
  }
...
router.get("/get_by_id/:id", async function(req, res, next) {
  try {
  ...
  if(req.body.title && req.body.description && req.body.status)
  {
    try {
      ...
      const result = await db.query(
        "INSERT INTO category_group (title,description,icon,status) VALUES ($1,$2,$3,$4) RETURNING *",
      ...
      if(result.rows[0].id) {
        const result1 =  db.query(
          ...
          [result.rows[0].id, req.body.category_ids]
        );
      }
  ...
  if(req.body.title)
  {
    try {
        ...
        if(req.body.icon!=null)
        {
          ...
          app_logo="header/"+Date.now()+"-header.png";
     ...
     var categoryIds = JSON.stringify(req.body.categories);
     console.log(categoryIds);
      if(result.rows[0].id) {
        const result1 =  db.query(
          "INSERT INTO category_group_relation (category_group_id,category_ids) VALUES ($1,$2) RETURNING *",
          [result.rows[0].id, categoryIds]
        );
      }
    ...
    try {
      ...
      if (req.body.isFile) {
         if(res_cat.rows[0].icon !='default/subject-1.png' && res_cat.rows[0].icon !='default/subject-2.png' && res_cat.rows[0].icon !='default/subject-3.png' && res_cat.rows[0].icon !='default/subject-4.png')
        {
         ...
         ///s3 upload
        var buf = Buffer.from(req.body.icon.replace(/^data:image\/\w+;base64,/, ""),'base64')
        var app_logo="header/"+Date.now()+"-header.png";
        s3.putObject({Bucket: 'allam-stg', Key: app_logo, Body: buf}, function(err, data) {
          if (err) {
            //return res.send({ success: false, err: err });
          } else {
...
console.log(req.body.id);
  if(req.body.id)
  {
    ...
    try {
        ...
        var q_category = await db.query("UPDATE category_group SET status=$1 WHERE id=$2",[category.status, category.id]);
...
router.get("/check_category", async function(req, res, next) {
  ...
  try {
    const results = await db.query("SELECT * FROM category_group WHERE lower(title)=$1", [title]);
